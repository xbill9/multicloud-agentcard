"""AWS card specimen: the a2a-sdk reference routes, nothing else.

Deployed to Bedrock AgentCore. Locally it is one of two stacks on the bench, and the
...
AWS card differs because of the runtime, not the SDK.

    python -m agents.aws.server

Environment: ``PORT`` (11002), ``PUBLIC_URL``, ``HOST``.
"""
